 ...
 *
 */
if ( ! class_exists( 'AVAN_Field_wp_editor' ) ) {
  class AVAN_Field_wp_editor extends AVAN_Fields {
    ...
    public function render() {
      ...
      echo ( avan_wp_editor_api() ) ? '<div class="avan-wp-editor" data-editor-settings="'. esc_attr( json_encode( $editor_settings ) ) .'">' : '';

      echo '<textarea name="'. esc_attr( $this->field_name() ) .'"'. $this->field_attributes( $attributes ) . $editor_height .'>'. $this->value .'</textarea>';

      echo ( avan_wp_editor_api() ) ? '</div>' : '';
    ...
    public function enqueue() {
      ...
      if ( avan_wp_editor_api() && function_exists( 'wp_enqueue_editor' ) ) {
    ...
    public function setup_wp_editor_media_buttons() {
      ...
      echo 'var avan_media_buttons = '. json_encode( $media_buttons ) .';';
      echo '</script>';
    ...
    public function setup_wp_editor_settings() {
      ...
      if ( avan_wp_editor_api() && class_exists( '_WP_Editors') ) {

        $defaults = apply_filters( 'avan_wp_editor', array(
          'tinymce' => array(
            'wp_skip_init' => true
          ),
        ) );

        $setup = _WP_Editors::parse_settings( 'avan_wp_editor', $defaults );

        _WP_Editors::editor_settings( 'avan_wp_editor', $setup );
